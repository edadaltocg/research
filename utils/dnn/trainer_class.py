...
class Trainer:
    ...
    Features:
        ...
        - Autorestart from last checkpoit
    ...
    effective_device_train_batch_size: Optional[int] = None
    ...
    max_duration_in_hours: Optional[float] = None
    ...
    def setup_model(self):
        if (
            self.fsdp
            and self.is_dist_avail_and_initialized()
    ...
    @staticmethod
    def setup_distrib_if_available(
        backend="nccl", master_addr="localhost", master_port="12355"
    ):
        """Run with torchrun:
    ...
    def optimizer_state_dict(self):
        if self.fsdp and self.is_dist_avail_and_initialized() and False:
            with FSDP.state_dict_type(
                self.optimizer, StateDictType.FULL_STATE_DICT, self.fsdp_optim_policy
            ):
    ...
    def save_checkpoint(self, prefix: Optional[str] = None):
        if self.is_dist_avail_and_initialized():
            # dist.barrier()
            pass
        if prefix is None:
    ...
    def setup_train_dataloader(self):
        ...
        if self.is_dist_avail_and_initialized():
            sampler = DistributedSampler(
                self.train_dataset, shuffle=True, drop_last=self.drop_last
            )
    ...
    def eval(self):
        ...
        for i, batch in enumerate(self.eval_dataloader):
            ...
            t1 = time.monotonic()
            self.metrics["eval/throughput"].update(
                (i + 1) * self._device_eval_batch_size, t1 - t0
            )
    ...
    def train(self):
        ...
        with profiler as p:
            for self.curr_step in range(self.start_step, self.num_steps):
                ...
                t1 = time.monotonic()
                self.metrics["train/throughput"].update(
                    (self.curr_step + 1) * self._device_eval_batch_size, t1 - t0
                )
                ...
                if self.should_eval() and False:
                    ...
                    self.log_metrics_to_console("eval", eval_metrics)
                    if (
                        eval_metrics["eval/loss"] < self.best_eval_loss
                        and self.checkpoint_best
                    ):
